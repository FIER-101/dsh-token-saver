...
//  - 错误率追踪：压缩导致的错误在滚动窗口内统计；飙升则自动优化压缩算法
//  - 用量统计：余额（官方 /user/balance）、当日花费、当前任务进程
//  - 启发式预测：高花费风险触发模型确认 → 全自动给出最优策略 + 成本预警提示
...
const MIN_COMPRESSIONS = 10;         // 至少多少次压缩才评估错误率
const ERROR_RATE_THRESHOLD = 0.3;    // 错误率阈值：超过则自动优化
const MAX_OPTIMIZATIONS = 3;         // 最多自动优化次数
const RISK_MEDIUM_PCT = 0.3;         // 今日花费占余额 30% → medium
...
async function apply(ctx) {
  ...
  const stats = {
    ...
    predictions: null, // { at, high_spend, forecast, recommendation }
    optimizationCount: 0,
    lastOptimize: null,
  };
  ...
  // 错误归因：工具报错且 30s 内发生过压缩 → 记为压缩相关错误
  ...
  function buildPredictionPrompt() {
    ...
    return [
      ...
      '- 今日已花费：¥' + stats.todaySpend + '（' + stats.todayTokens + ' tokens）',
      '- 当前会话累计：' + stats.totalTokens + ' tokens，上下文 ' + stats.surfaceTokens + '/' + stats.contextWindow,
  ...
  if (systemPrompt && typeof systemPrompt.section === 'function') {
    systemPrompt.section({
      ...
      text: () => {
        ...
        return '⚠️【Token 管家成本预警】当前任务预计会消耗较多余额。请先收敛范围、减少冗长中间步骤，必要时先做检查点或停下来与用户确认，不要盲目继续。'
          + (rec ? ' 最优策略建议：' + rec : '');
      },
    });
  }
