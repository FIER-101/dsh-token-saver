...
async function apply(ctx) {
  ...
  // 当前任务上下文基线：新模型请求到来时重置 → 进度条从零随任务增长
  let taskBaseline = { sessionId: null, surface: null };
  ctx.on('session/event', (session, event) => {
    if (event && event.type === 'request/header' && session && tm) {
      try { const m = tm.measure(session); taskBaseline = { sessionId: session.id, surface: m ? (m.surfaceTokens ?? 0) : 0 }; } catch (e) { /* ignore */ }
    }
  });
  ...
  if (sp && typeof sp.onChanged === 'function') {
    sp.onChanged((_session, key, value, _seq) => {
      if (key === 'tokenUsage' && value && typeof value.cacheReadTokens === 'number') {
        stats.providerCacheSaved = value.cacheReadTokens;
      }
    });
  }
  ...
  function creditPlan() {
    if (!pendingPlan || pendingPlan.savedTokens <= 0) { pendingPlan = null; return; }
    const saved = Math.round(pendingPlan.savedTokens);
    const timeMs = Math.max(1, Date.now() - (pendingPlan.at || Date.now()));
    stats.planSavedTokens += saved;
    stats.lastPlanSaved = { at: Date.now(), savedTokens: saved, anchorSpend: pendingPlan.anchorSpend, timeMs };
  ...
  const refreshStats = () => {
    ...
    if (session) {
      ...
      const base = taskBaseline && taskBaseline.sessionId === session.id && taskBaseline.surface != null ? taskBaseline.surface : null;
      stats.contextSurface = base == null ? maxSurf : Math.max(0, maxSurf - base);
      stats.contextUsagePct = Math.min(100, Math.round(stats.contextSurface / stats.contextWindow * 100));
      try {
