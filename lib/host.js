...
const DATA_REPO = 'FIER-101/dsh-token-data'; // 数据推送仓库（仅当显式开启 TOKEN_SAVER_PUSH_REMOTE=on 时推送）
const PUSH_INTERVAL_MS = 60 * 1000;  // 推送间隔
const HISTORY_CAP = 500;             // 历史快照条数上限
const HISTORY_FILE = dshHome + '/data/token-saver-history.jsonl'; // 本地正反馈/用量历史副本（token 管家可读）
const REMOTE_PUSH = (typeof process !== 'undefined' && process.env && process.env.TOKEN_SAVER_PUSH_REMOTE === 'on'); // 远端推送默认关闭（隐私）
...
// 按任务意图启发式估算需要的步数（可再接入模型精调）
function estimateSteps(text) {
  if (!text) return 8;
  const t = text.toLowerCase();
  let steps = 5;
  const heavy = ['测试', '实现', '开发', '修复', '重构', '迁移', '多个文件', '项目', '全套', '端到端', '集成', '接口', '数据库', '前端', '后端'];
  const medium = ['分析', '总结', '文档', '审查', '检查', '调试', '对比', '优化', '方案', '调研'];
  for (const w of heavy) if (t.includes(w)) { steps += 3; break; }
  for (const w of medium) if (t.includes(w)) { steps += 1; break; }
  if (t.length > 300) steps += 1;
  if (t.length > 800) steps += 2;
  if (t.length > 1500) steps += 2;
  return Math.max(3, Math.min(16, steps));
}
...
async function apply(ctx) {
  ...
  // 压缩开关（内存态）
  let enabled = true;
  // 排除名单：这些工具的结果通常需要完整查看，跳过压缩（read/write/edit 保源码完整，检索类 glob/grep 可压缩以真正省 token）。
  ...
  // 压缩参数（自动优化会动态调整）
  let textLimit = 4000;        // 超过该总字符数触发压缩（按"省 token"初衷下调）
  let headRatio = 0.4;
  let tailRatio = 0.25;
  const MIN_BLOCK = 1200;      // 单块低于该长度不压缩（多块合计超限时也能真正触发）

  // 压缩事件窗口（错误率）
  const compressEvents = []; // { at, tool, errored }
  let recentCompression = null;
  let agentsRunning = 0; // 运行中的 agent 数（任务结束判定）
  let lastAgentActivity = Date.now(); // 最近一次 agent 活动时刻（防计数卡死兜底）
  let taskSteps = 0;     // 当前任务已执行步骤数
  let taskCompletedAt = 0;     // 上次任务完成时刻（idle 判"完成绿"）
  let approvalPendingUntil = 0; // 待授权 → 黄
  let blockedUntil = 0;         // 出错受阻 → 红
  ...
  const stats = {
    ...
    taskCount: 0,        // 活跃任务数
    taskStatus: 'none',  // none|running|complete|blocked|waiting
    balanceCap: 100,
    // 节省 / 错误率 / 风险
    savedChars: 0,
    savedTokens: 0,
  ...
  let lastStateSave = 0; // 状态文件写盘节流（避免每 2s 写盘）
  ...
  // 每个任务(会话)一条进度条：新用户消息(新任务)到来时重置该任务基线 + 规划步数
  const taskBaselines = new Map(); // sessionId -> 任务开始时的 surface
  const taskPlans = new Map();     // sessionId -> { plannedSteps, completedSteps, active }
  ctx.on('session/event', (session, event) => {
    if (event && event.type === 'user/message' && session && tm) {
      ...
      // 任务开始：先规划需要几步，再跑任务（分段式进度按计划步数）
      try {
        const txt = textOfMessage(event.message);
        taskPlans.set(session.id, { plannedSteps: estimateSteps(txt), completedSteps: 0, active: true });
      } catch (e) { /* ignore */ }
    }
  });
  ...
  function refreshToday() {
    ...
    const dayChanged = dayKey !== now;
    if (dayChanged) { dayKey = now; baselineTokens = null; dayStartBalance = null; }
    ...
    // 写盘节流：日切换立即保存，否则最多每 30s 一次，避免每 2s 高频写盘
    if (dayChanged || Date.now() - lastStateSave > 30000) {
      ...
      if (enabled && exec && !EXCLUDE.has(exec.name)
        ...
        if (hasText && total > textLimit) {
          const pruned = result.content.map((b) => {
            if (b && b.type === 'text' && typeof b.text === 'string' && b.text.length > MIN_BLOCK) {
              const t = b.text;
  ...
  // 压缩归因辅助：标记该工具在窗口内最近一次未标记的压缩事件（修复"只统计最近一次"导致错误率失真）
  function markCompressionErrored(tool, windowMs) {
    for (let i = compressEvents.length - 1; i >= 0; i--) {
      const ev = compressEvents[i];
      if (ev.tool === tool && Date.now() - ev.at < windowMs && !ev.errored) { ev.errored = true; return; }
    }
  }
  ...
  ctx.on('tools/result', (_exec, result) => {
    ...
    // 错误归因 A：工具结果报错且窗口内有该工具的压缩事件 → 标记最近一次为压缩相关错误
    if (result && result.isError) {
      const tool = _exec && _exec.name;
      if (tool) markCompressionErrored(tool, 30000);
    }
  });
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    try {
      if (exec && exec.name) markCompressionErrored(exec.name, RETRY_WINDOW_MS);
    } catch (e) { /* ignore */ }
    return next();
  });
  ...
  ctx.on('agent/status', (payload) => {
    try {
      ...
      else if (st === 'idle') { agentsRunning = Math.max(0, agentsRunning - 1); if (agentsRunning === 0) taskCompletedAt = Date.now(); }
    } catch (e) { /* ignore */ }
  });
  // 出错受阻 → 红（短窗口）
  ctx.on('agent/error', () => { try { blockedUntil = Date.now() + 10000; } catch (e) { /* ignore */ } });
  // 请求授权 → 黄（短窗口）
  ctx.on('approval/request', (_req, next) => { try { approvalPendingUntil = Date.now() + 20000; } catch (e) { /* ignore */ } return next(); });
  // 每执行一步推进任务进度（waterfall：必须调用并返回 next）
  ctx.on('agent/pre-step', (payload, next) => {
    try {
      lastAgentActivity = Date.now();
      taskSteps += 1;
      const sid = payload && payload.agent && payload.agent.session && payload.agent.session.id;
      if (sid && taskPlans.has(sid)) taskPlans.get(sid).completedSteps += 1;
    } catch (e) { /* ignore */ }
    return next();
  });
  ...
  function maybeOptimize() {
    ...
    if (compressionErrRate > ERROR_RATE_THRESHOLD) {
      ...
      stats.lastOptimize = { at: Date.now(), errorRate: compressionErrRate, textLimit, headRatio, tailRatio, excludedTool: tool || null };
  ...
  let predicting = false; // 预测请求防重入：避免每 2s 的付费 API 请求风暴
  async function predictWithModel() {
    if (predicting || !creds) return;
    ...
    } catch (e) { /* 失败同样推进冷却，避免每 2s 重试轰炸 */ stats.predictions = { at: Date.now(), high_spend: false, forecast: null, recommendation: null }; }
  ...
  function pushHistory() {
    ...
    // 本地持久化副本：正反馈/用量历史本地也有一份，供 token 管家读取，不依赖远端
    try {
      mkdirSync(dshHome + '/data', { recursive: true });
      writeFileSync(HISTORY_FILE, JSON.stringify(history[history.length - 1]) + '\n', { flag: 'a', encoding: 'utf8' });
    } catch (e) { /* 存储失败静默 */ }
  }
  ...
  function buildAnalogyReminders() {
    ...
    // 3) 已有能力/官方思路未被发现（短文本，避免每轮大段自我推销）
    parts.push('【能力提醒】先检查是否已有现成 skill/工具可用，勿手写或臆造；优先复用已有方案。');
    return parts;
  }
  ...
  // ─── 周期任务 ───
  const refreshStats = () => {
    // 兜底：若 agent 异常退出（未发 idle 事件）导致计数卡住，超过 45s 无活动则视为任务已结束
    if (agentsRunning > 0 && Date.now() - lastAgentActivity > 45000) {
      agentsRunning = 0;
      ...
      for (const s of sessions.list()) {
        try {
          ...
          if (m) {
            ...
            // 任务进度：先规划步数，按已完成/计划步数分段推进；结束=100%
            const plan = taskPlans.get(s.id);
            const planned = (plan && plan.plannedSteps) || 8;
            const done = (plan && plan.completedSteps) || 0;
            const pct = taskDone
              ? 100
              : Math.min(100, Math.max(5, Math.round(done / planned * 100)));
            tasks.push({
              sessionId: s.id,
              surface: surf,
    ...
    if (session) {
      ...
      // 任务进度：任务结束(全 agent 空闲)=100%；进行中按 已完成/计划步数 分段推进
      stats.taskRunning = !taskDone;
      stats.taskSteps = taskSteps;
      const mainPlan = main && main.plannedSteps ? main.plannedSteps : (taskPlans.get(session.id) ? taskPlans.get(session.id).plannedSteps : 8);
      const mainDone = main && main.completedSteps != null ? main.completedSteps : 0;
      stats.plannedSteps = mainPlan;
      stats.completedSteps = mainDone;
      stats.contextUsagePct = taskDone
        ? 100
        : Math.max(stats.contextUsagePct, Math.min(100, Math.round(mainDone / mainPlan * 100)));
      // 任务状态灯：进行中(受阻红/待授权黄)/完成绿/无任务灰
      if (agentsRunning > 0) {
        stats.taskStatus = (Date.now() < blockedUntil) ? 'blocked'
          : (Date.now() < approvalPendingUntil) ? 'waiting'
          : 'running';
      } else {
        stats.taskStatus = (Date.now() - taskCompletedAt < 30000) ? 'complete' : 'none';
      }
      try {
    ...
    // 正反馈收敛判定：曾给最优方案，如今风险非 high 且余额可查 → 记功（避免余额断连时虚构记账）
    if (pendingPlan && stats.risk !== 'high' && stats.balance && stats.balance.available) creditPlan();
    // 智能节省合计
  ...
  if (timer && typeof timer.interval === 'function') {
    ...
    timer.interval(() => { pushHistory(); if (REMOTE_PUSH) void pushToRepo(); }, PUSH_INTERVAL_MS);
  }
  ...
  // ─── token_switch 工具：开启/关闭工具输出智能压缩（README 承诺的"可开关"落地） ───
  ctx.tools.register(defineTool({
    name: 'token_switch',
    description: '开启或关闭 token 管家的工具输出智能压缩。传 enabled=true/false；不传则仅返回当前状态与压缩参数。',
    parameters: { enabled: { type: 'boolean', description: 'true=开启压缩，false=关闭压缩；省略则只查询。' } },
    output: { schema: { type: 'json' }, render: (_a, v) => [{ type: 'text', text: JSON.stringify(v, null, 2) }] },
    async execute(args) {
      if (typeof args.enabled === 'boolean') enabled = args.enabled;
      stats.enabled = enabled;
      return { enabled, textLimit, headRatio, tailRatio, excludedTools: [...EXCLUDE] };
    },
  }));
