 ...
 * 分区：连接状态（点阵猫头像：成功/失败/红温发怒 + 状态脉冲点）、余额、当日花费（官方）、
 * 本次花费、当前任务进程、预测节省、错误率、用量。
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// —— 点阵猫（12×12 细化：. 透明，# 身体，E 眼，M 嘴，T 泪）——
		// 顶部两角凸起三角 = 耳朵
		const SMILING_CAT = [
			"..#......#..",
			".##......##.",
		...
		function DotAvatar({ variant }) {
			...
			return React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(12, 6px)", gap: 1, width: "fit-content", animation: cat.anim } },
				cat.grid.flatMap((row, r) =>
					row.split("").map((ch, c) => {
						...
						return React.createElement("div", { key: r + "-" + c, style: { width: 6, height: 6, borderRadius: "50%", background: bg } });
					})
				)
			);
		}
